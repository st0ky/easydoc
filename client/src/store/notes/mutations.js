...
export const newNote = function (
  ...
  var newNote = {
    ...
    edited: edited == null ? Date.now() : edited
  }
...
export const newTree = function (state, tree) {
  const newTree = {}
  let tmp_obj = { ...tree }
  tmp_obj.parent = null
  const queue = [tmp_obj]
  ...
  tmp_obj = { note: -1, parent: tree.note, children: children }
  newTree[-1] = tmp_obj
  newTree[tree.note].children.push(tmp_obj)
  Vue.set(state.trees, tree.note, newTree)
