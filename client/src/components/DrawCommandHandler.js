...
DrawCommandHandler.prototype.doKeyDown = function() {
  ...
  if (
    selected &&
    e.key == "\r" &&
    Number.isInteger(selected.data.parent) &&
    !(e.control || e.meta)
  ) {
    e.bubbles = false;
...
DrawCommandHandler.prototype.deleteSelection = function() {
  ...
  if (titles.length == 1) titles = titles[0];
...
DrawCommandHandler.prototype._arrowKeyTree = function() {
  ...
  diagram.selection.each(function(p) {
    if (p instanceof go.Node) selected = p;
  });
  ...
  if (
    (e.key === "Right" && selected.data.dir == "right") ||
    (e.key === "Left" &&
      (selected.data.dir === undefined || selected.data.dir == "left"))
  ) {
